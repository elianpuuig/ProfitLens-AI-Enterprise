...
def ejecutar_consultoria_ia(v_actual, v_pred, vips, clave_reporte):
    """Lógica que usa st.session_state para persistir el reporte sin gastar API."""
    
    # 1. Si ya existe un reporte para esta clave, lo mostramos directo y salimos
    if st.session_state.get(clave_reporte):
        with st.chat_message("assistant", avatar="💎"):
            st.markdown(st.session_state[clave_reporte])
        return

    # 2. Si no existe, lo pedimos a la IA
    with st.chat_message("assistant", avatar="💎"):
        with st.spinner("Consultor ProfitLens analizando con IA..."):
            ...
            try:
                ...
                # Pedimos a la función con caché (VITAL)
                respuesta = pedir_analisis_ia(prompt)
                
                # Guardamos permanentemente en la sesión para que no se borre al recargar
                st.session_state[clave_reporte] = respuesta
                st.markdown(respuesta)
                
            except Exception as e:
                st.error("🚀 **ProfitLens AI:** El motor está saturado. Esperá 60 segundos y reintentá.")             
...
def main():
    ...
    if st.session_state.df is not None:
        ...
        with tab2:
            try:
                ...
                if st.button("🧠 Consultoría Estratégica", key="btn_pareto"):
                    # LLAMADA LIMPIA: Solo una línea
        ...
        with tab3:
            try:
                ...
                if st.button("🧠 Plan de Acción Forecast", key="btn_forecast"):
                    # Obtenemos los 3 principales productos para el contexto
    ...
    st.divider()
    st.caption("© 2026 ProfitLens AI Enterprise - Elián Puig | Puerto Madryn")
